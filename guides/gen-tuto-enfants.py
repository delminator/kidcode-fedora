...
def slug(s):
    """Nom de fichier sans accent ni majuscule (ex: 'Été Génial' -> 'ete-genial')."""
    s = unicodedata.normalize("NFKD", s).encode("ascii", "ignore").decode()
    return re.sub(r"[^a-z0-9]+", "-", s.lower()).strip("-")

# compte -> { prénom affiché, sections "passion" à ajouter }
ENFANTS = {
    # version publique NEUTRE « pour tous » (aucun prénom)
    "eleve": {"nom": "tous les enfants", "bonus": ["speleo", "musique"]},
}
...
touche <b>AltGr</b> (à droite de la barre d'espace). Demande à papa de te les montrer, et
...
<p>Plein de musique est <b>gratuite et libre</b> (tu as le droit de la prendre !). Demande à papa
...
possède déjà. Télécharger de la musique payante sans payer, c'est interdit — en cas de doute, demande à papa.</div>
...
programme Python qui dit où est la <b>Lune</b> ce soir ! (cherche « python ephem moon » avec papa)</div>
...
tutos intégrés</b>. Il s'installe d'une façon spéciale → <b>demande à papa</b> de le mettre en place
...
BONUS = {"speleo": SPELEO_HTML, "musique": MUSIQUE_HTML}


def build(account, info):
    disp = info["nom"]
    out = TEMPLATE.replace("%%NAME%%", disp).replace("%%USER%%", account)
    # sections "passion" propres à cet enfant
    bonus = "".join(BONUS[k] for k in info.get("bonus", []) if k in BONUS)
    ...
    for key in CODES:
        ...
        expl = ""
        if key in EXPL:
            expl = ('<div class="expl">🔍 <b>Ce que fait le code :</b><ul>'
                    + EXPL[key] + '</ul></div>')
        out = out.replace(block, f'<pre class="code">{C(key)}</pre>{expl}')
    path = f"tuto-{slug(disp)}.html"
...
if __name__ == "__main__":
    ...
    for account, info in ENFANTS.items():
        print("écrit :", build(account, info))
